 ...
 * @author     Jan Skrasek, Josef Šíma
 * 
 */

namespace Yosieu\Datagrid;
...
class Datagrid extends UI\Control
{
    ...
    public function render($tostring = false)
    {
        ...
        if($this->noform){
            $this->template->setFile(__DIR__ . '/Datagrid.noform.latte');
        }else{
    ...
    {
        if (!$this->data) {
            if(!is_null($this->paginator) && $this->paginator->getPage() == 1) {
                $this->paginator->setPage($this->getStoredPaginator());
            }

            $orderingData = $this->getStoredOrder();
            if(is_null($this->orderColumn) && !empty($orderingData) ){
                $this->orderColumn = $orderingData['orderColumn'];
                $this->orderType = $orderingData['orderType'];
            }
    ...
    public function createComponentForm()
    {
        ...
        if ($this->filterFormFactory) {
            ...
            if (!$this->filterDataSource) {
                ...
                $storedFilterData = $this->getStoredFilterData();

                if( !empty($storedFilterData) ) {

                    $this->filterDataSource = $storedFilterData;
                    $form['filter']->setValues($this->filterDataSource);
                }
            }
        }
    ...
    public function processForm(UI\Form $form)
    {
        ...
        if (isset($form['filter'])) {
            if ($form['filter']['filter']->isSubmittedBy()) {
                ...
                $this->setStoredFilterData($this->filter);
                $this->invalidateControl('rows');
            } elseif ($form['filter']['cancel']->isSubmittedBy()) {

                $this->resetStoredFilterData();
                if ($this->paginator) {
                    $this->page = $this->paginator->page = 1;
                }
    ...
    /**
     * If setted to TRUE, then is used alternative template, with no form elements
     * @param type $trueOrFalse
     */
    public function setNoFormRender($trueOrFalse) {
        $this->noform = $trueOrFalse;
    }
    ...
    protected function getSessionName() {
        ...
        $loc = 'grid-filter:'.get_class($this->presenter).":".$this->name;
    ...
    /* STORE FILTER */
    protected function resetStoredFilterData() {
        if(isset($_SESSION[$this->getSessionName()]['filter'])){
            unset($_SESSION[$this->getSessionName()]['filter']);
        }
    }

    protected function getStoredFilterData() {
        if(isset($_SESSION[$this->getSessionName()]['filter']) && $this->storeFilter){
            return $this->filterFormFilter($_SESSION[$this->getSessionName()]['filter']);
        }else {
            return Array();
        }
    }

    protected function setStoredFilterData($values) {
        if($this->storeFilter){
            $_SESSION[$this->getSessionName()]['filter'] = $this->filterFormFilter($values);
        } else {
            $this->resetStoredFilterData();
        }
    }
    ...
    /* STORE PAGINATOR */
    protected function resetStoredPaginator() {

        if(isset($_SESSION[$this->getSessionName()]['paginate'])){
            unset($_SESSION[$this->getSessionName()]['paginate']);
        }
    }

    protected function getStoredPaginator() {

        if(isset($_SESSION[$this->getSessionName()]['paginate']) && $this->storeFilter){
            return $_SESSION[$this->getSessionName()]['paginate'];
        }else {
            return 1;
        }
    }

    protected function setStoredPaginator() {
        if($this->storeFilter){
            $_SESSION[$this->getSessionName()]['paginate'] = $this->paginator->getPage();
        } else {
            $this->resetStoredPaginator();
        }
    }

    /* STORE ORDER */
    protected function resetStoreOrder() {

        if(isset($_SESSION[$this->getSessionName()]['order'])){
            unset($_SESSION[$this->getSessionName()]['order']);
        }
    }

    protected function getStoredOrder() {
        if(isset($_SESSION[$this->getSessionName()]['order']) && $this->storeFilter){
            return $_SESSION[$this->getSessionName()]['order'];
        }else {
            return Array();
        }
    }

    protected function setStoredOrder() {
        if($this->storeFilter && !is_null($this->orderColumn)){
            $_SESSION[$this->getSessionName()]['order'] = Array('orderColumn' => $this->orderColumn, 'orderType' => $this->orderType);
        } else {
